...
function getCertaintyColor(c: number | undefined): string {
  if (c == null) return COLORS.gray;
  if (c > 90) return COLORS.green;
  if (c >= 50) return COLORS.orange;
  return COLORS.red;
}

function getCertaintyLabel(c: number | undefined): string {
...
interface ExplanationPanelProps {
  ...
  overallCertainty: number | undefined;
...
function CertaintyBadge({ value, large }: CertaintyBadgeProps) {
  ...
  return (
    <span
      role="status"
      aria-label={value != null ? `Certainty: ${value}%, ${getCertaintyLabel(value)}` : "Certainty: Unverified"}
      style={{
        display: "inline-flex",
        alignItems: "center",
        gap: 4,
...
function FindingSpan({ finding, isActive, onActivate }: FindingSpanProps) {
  ...
  return (
    <span
      role="button"
      tabIndex={0}
      aria-label={`Finding: ${finding.text}. ${finding.certainty != null ? `Certainty ${finding.certainty}%` : "Unverified"}. Click for explanation.`}
        ...
        {isOverview && (
          <>
            <p style={{ fontSize: 13, lineHeight: 1.75, color: COLORS.textSecondary, margin: "0 0 4px", whiteSpace: "pre-wrap" }}>
              {overviewData?.explanation?.text || (overallCertainty != null
                ...
                : `This report was generated by DoublyAI using a multi-agent pipeline: Research Agent gathered evidence and Synthesis Agent drafted findings. Verification was skipped — findings have not been adversarially reviewed and certainty scores are not available.`)}
            </p>
...
export default function Report({ data, traceData, onBack, publishedSlug }: ReportProps) {
  ...
  const overallCertainty = useMemo<number | undefined>(() => {
    if (meta?.overallCertainty != null) return meta.overallCertainty;
    const withCertainty = safeFindings.filter((f) => f.certainty != null);
    if (withCertainty.length === 0) return undefined;
    return Math.round(withCertainty.reduce((s, f) => s + f.certainty!, 0) / withCertainty.length);
      ...
      <div style={{ flex: 1, overflow: "auto" }}>
        <div style={{ maxWidth: 780, margin: "0 auto", padding: isMobile ? "20px 16px 60px" : "32px 40px 60px" }}>
          ...
          <div
            ...
            role="button"
            tabIndex={0}
            aria-label={overallCertainty != null ? `Overall report certainty: ${overallCertainty}%. Click for methodology.` : "Report unverified. Click for methodology."}
          ...
          >
            ...
            <span style={{ display: "flex", alignItems: "center", gap: 4 }}>
              <span style={{ width: 8, height: 8, borderRadius: "50%", background: COLORS.gray, display: "inline-block" }} />
              Unverified
            </span>
            {!isMobile && (
